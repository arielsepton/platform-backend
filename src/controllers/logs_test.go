...
func TestFetchPodLogs(t *testing.T) {
	...
	cases := map[string]struct {
		...
		"ShouldFailGettingLogsOnNonExistingPod": {
			args: args{
				...
				containerName: testutils.ContainerName,
			},
			want: want{
	...
	setup()
	mocks.CreateTestPod(fakeClient, "test-namespace", "test-pod-1", "", false)
...
func TestFetchCappLogs(t *testing.T) {
	...
	setup()
	mocks.CreateTestPod(fakeClient, "test-namespace", "test-pod-1", "test-capp", false)
	mockLogger, _ := zap.NewDevelopment()
