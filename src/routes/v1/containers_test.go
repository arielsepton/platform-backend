	...
	cases := map[string]struct {
		...
		"ShouldSucceedGettingContainers": {
			...
			want: want{
				...
				response:   map[string]interface{}{"containers": []interface{}{map[string]interface{}{"containerName": "test-container"}, map[string]interface{}{"containerName": "test-capp"}}, "count": 2},
			},
		},
		"ShouldHandleNotFoundPod": {
			...
			want: want{
				...
				response:   map[string]interface{}{"details": fmt.Sprintf("failed to get pod %q, in the namespace %q with error: pods %q not found", pod1+testutils.NonExistentSuffix, testNamespaceName, pod1+testutils.NonExistentSuffix), "error": "Operation failed"},
			},
		},
		"ShouldHandleNotFoundNamespace": {
			...
			want: want{
				statusCode: http.StatusNotFound,
				response:   map[string]interface{}{"details": fmt.Sprintf("failed to get pod %q, in the namespace %q with error: pods %q not found", pod1, testNamespaceName+testutils.NonExistentSuffix, pod1), "error": "Operation failed"},
			},
		},
	}
